...
import com.sopt.now.container.repository.FollowerRepositoryImpl
...
class SoptAppContainer(
    ...
    override val authAfterLoinRepository: AuthRepository by lazy {
        val authService = create<AuthService>(retrofitAfterLogin)
        AuthRepositoryImpl(authService)
    }

    override val followerRepository: FollowerRepository by lazy {
        val followerService = create<FollowService>(retrofitFollow)
        FollowerRepositoryImpl(followerService)
    }

    private val retrofitBeforeLogin: Retrofit by lazy {
        Retrofit.Builder()
            .baseUrl(BASE_URL)
            .addConverterFactory(Json.asConverterFactory("application/json".toMediaType()))
            .build()
    }
    private val retrofitFollow: Retrofit by lazy {
        Retrofit.Builder()
    ...
    private val retrofitAfterLogin: Retrofit by lazy {
        Retrofit.Builder()
            .baseUrl(BASE_URL)
            .addConverterFactory(Json.asConverterFactory("application/json".toMediaType()))
    ...
    private fun createHttpClient(): OkHttpClient.Builder {
        val sharedPreferences =
            context.getSharedPreferences(LoginActivity.PREFERENCE_KEY, Context.MODE_PRIVATE)
        val memberId =
            sharedPreferences.getString(LoginActivity.MEMBER_ID_KEY, "")?.ifEmpty { "" }!!
